...
public class DemoController {
	...
	@PostMapping(path = "/echo", 
			consumes= {MediaType.APPLICATION_STREAM_JSON_VALUE, MediaType.TEXT_EVENT_STREAM_VALUE},
			produces={MediaType.APPLICATION_STREAM_JSON_VALUE, MediaType.TEXT_EVENT_STREAM_VALUE})
	public Flux<String> upperCase(@RequestBody Flux<String> body) {
		return body.map(String::toUpperCase).map(x -> x + "\n");
	}
